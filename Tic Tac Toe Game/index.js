...
minmax.prototype.minMove = function (board) {
  if (this.won(this.minPlayer, board)) {
    ...
    let leastValue = 100;
    for (let i = 0; i < board.length; i++) {
      let newBoard = this.makeMove(i, board, this.minPlayer);
      if (newBoard) {
        let value = this.maxMove(newBoard);
        if (value < leastValue) {
          leastValue = value;
        }
      }
...
minmax.prototype.maxMove = function (board) {
  if (this.won(this.minPlayer, board)) {
    ...
    let maxValue = -100;
    for (let i = 0; i < board.length; i++) {
      let newBoard = this.makeMove(i, board, this.maxPlayer);
      if (newBoard) {
        let value = this.minMove(newBoard);
        if (value > maxValue) {
          maxValue = value;
        }
      }
...
/*In this function we have to check whether the player in the current board configuration has won the game or not. For a win we have to check the row , columns and diagnols respectively
 */
...
function delayComputerMove() {
  let move = game.minMaxAlgo(board);
...
$("#x").click(function () {
  ...
  $("#options").toggle(1000);
  computerChoice = "<i class='fa fa-circle-o animated flip'></i>";
...
function delayO() {
  ...
  userChoice = "<i class='fa fa-circle-o animated flip'></i>";
   ...
   */
  let move = Math.floor(Math.random() * (myMax - myMin + 1)) + myMin;
  board[move] = game.maxPlayer;
  let divId = convertValueToDiv(move);
